...
#
# If your core_igraph expects “no edge = 0”, keep use_inf_off_block=False (default).
#
...
#
import numpy as np
from core_igraph import dijkstra_igraph_all, dijkstra_igraph_to_target
...
dijkstra_igraph_all(D, 0, encrypted_node_namesD)
dijkstra_igraph_to_target(D, 0, 4, encrypted_node_namesD)
...
print("🔐 Encrypted node names:")
for k, v in encrypted_node_namesD1.items():
    print(f"{k}: {v}")

# Run Dijkstra from node 0 to all others
dijkstra_igraph_all(D1, 0, encrypted_node_namesD1)
dijkstra_igraph_to_target(D1, 0, 15, encrypted_node_namesD1)
